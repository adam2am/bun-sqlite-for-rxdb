...
### Added
- **$all operator** (MongoDB-compatible)
  - SQL translation with support for scalar values, regex patterns, and $elemMatch
  - JS fallback matcher for complex queries
  - Strict array-only behavior matching Mingo (no scalar field handling)
  - Handles empty arrays (returns no matches), regex patterns, and $elemMatch conditions
  - Comprehensive unit tests and property-based edge case coverage
- **BSON type guards for comparison operators**
  - Prevents type coercion bugs in $gt/$gte/$lt/$lte operators
  - Distinguishes arrays from objects (Array.isArray check)
  - Rejects cross-type comparisons (e.g., '80' > 50 now returns false)
  - Enforces strict BSON type boundaries matching MongoDB behavior

### Fixed 🔥
- **$exists operator array traversal bug**
  - Added $exists to structural operators list
  - Prevents incorrect traversal into array elements
  - Query `{tags: {$exists: true}}` on `tags: []` now correctly returns true

### Changed
- **Property-based test coverage expanded**
  - Added edge case arbitraries: empty arrays, regex patterns, duplicates, type mismatches
  - Added array vs object comparison tests for type guard validation
  - 1000 random queries + 10k stress test coverage

### Technical Details
- All 666 tests passing (100%)
- Zero regressions
- Matches MongoDB/Mingo behavior: strict array-only $all, no cross-type comparisons
- Atomic commits: 3 commits (operator implementation, test schema fix, version bump)
