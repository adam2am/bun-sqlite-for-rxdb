...
describe('$regex Operator', () => {
	it('translates simple pattern to LIKE', () => {
		...
		expect(result?.sql).toBe("(name LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.name') WHERE (type = 'text' AND value LIKE ? ESCAPE '\\')))");
	...
	it('translates end anchor to LIKE', () => {
		...
		expect(result?.sql).toBe("(email LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.email') WHERE (type = 'text' AND value LIKE ? ESCAPE '\\')))");
	...
	it('translates contains pattern to LIKE', () => {
		...
		expect(result?.sql).toBe("(description LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.description') WHERE (type = 'text' AND value LIKE ? ESCAPE '\\')))");
	...
	it('escapes LIKE special chars', () => {
		...
		expect(result?.sql).toBe("(username LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.username') WHERE (type = 'text' AND value LIKE ? ESCAPE '\\')))");
	...
	it('handles case-insensitive with COLLATE NOCASE', () => {
		...
		expect(result?.sql).toBe("(LOWER(name) LIKE LOWER(?) ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.name') WHERE (type = 'text' AND LOWER(value) LIKE LOWER(?) ESCAPE '\\')))");
		expect(result?.args).toEqual(['%john%', '%john%']);
	});
	...
	it('handles case-insensitive suffix pattern', () => {
		...
		expect(result?.sql).toBe("(LOWER(email) LIKE LOWER(?) ESCAPE '\\' OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.email') WHERE (type = 'text' AND LOWER(value) LIKE LOWER(?) ESCAPE '\\')))");
		expect(result?.args).toEqual(['%@gmail.com', '%@gmail.com']);
	});

	it('handles case-insensitive exact match', () => {
		const result = translateRegex('code', '^ABC$', 'i', mockSchema, 'code');
		expect(result?.sql).toBe("(LOWER(code) = LOWER(?) OR EXISTS (SELECT 1 FROM jsonb_each(data, '$.code') WHERE (type = 'text' AND LOWER(value) = LOWER(?))))");
