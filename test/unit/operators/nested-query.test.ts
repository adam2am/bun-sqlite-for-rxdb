...
describe('Nested Query Builder - Depth Tracking', () => {
	...
	expect(result).not.toBeNull();
	expect(result!.sql).toBe(
		"json_extract(data, '$.name') IS NOT NULL AND (((json_type(data, '$.age') IN ('integer', 'real') AND json_extract(data, '$.age') >= ?) AND (json_type(data, '$.status') = 'text' AND json_extract(data, '$.status') = ?)) OR (json_type(data, '$.role') = 'text' AND json_extract(data, '$.role') = ?))"
	);
	...
	expect(result).not.toBeNull();
	expect(result!.sql).toBe(
		"(((json_type(data, '$.country') = 'text' AND json_extract(data, '$.country') = ?) AND (((json_type(data, '$.age') IN ('integer', 'real') AND json_extract(data, '$.age') >= ?) AND (json_type(data, '$.age') IN ('integer', 'real') AND json_extract(data, '$.age') <= ?)) OR (json_type(data, '$.status') = 'text' AND json_extract(data, '$.status') = ?))) OR (json_type(data, '$.role') = 'text' AND json_extract(data, '$.role') = ?))"
	);
