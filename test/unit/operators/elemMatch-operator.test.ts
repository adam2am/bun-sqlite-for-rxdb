...
describe('$elemMatch Operator', () => {
	it('translates simple equality match', () => {
		...
		expect(result?.sql).toBe('EXISTS (SELECT 1 FROM jsonb_each(tags) WHERE COALESCE((value = ?), 0))');
	...
	it('translates object with multiple conditions', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(items) WHERE COALESCE((json_extract(value, '$.price') > ? AND json_extract(value, '$.qty') >= ?), 0))");
	...
	it('translates nested conditions', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(awards) WHERE COALESCE((json_extract(value, '$.award') = ? AND json_extract(value, '$.year') > ?), 0))");
	...
	it('translates $and operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE COALESCE((json_extract(value, '$.status') = ?), 0) AND COALESCE((json_extract(value, '$.age') > ?), 0))");
	...
	it('translates $or operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE COALESCE((json_extract(value, '$.status') = ?), 0) OR COALESCE((json_extract(value, '$.status') = ?), 0))");
	...
	it('translates $nor operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE NOT (COALESCE((json_extract(value, '$.status') = ?), 0) OR COALESCE((json_extract(value, '$.status') = ?), 0)))");
