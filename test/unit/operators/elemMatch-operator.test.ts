...
describe('$elemMatch Operator', () => {
	it('translates simple equality match', () => {
		...
		expect(result?.sql).toBe('EXISTS (SELECT 1 FROM jsonb_each(tags) WHERE value = ?)');
	...
	it('translates object with multiple conditions', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(items) WHERE json_extract(value, '$.price') > ? AND json_extract(value, '$.qty') >= ?)");
	...
	it('translates nested conditions', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(awards) WHERE json_extract(value, '$.award') = ? AND json_extract(value, '$.year') > ?)");
	...
	it('translates $and operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE json_extract(value, '$.status') = ? AND json_extract(value, '$.age') > ?)");
	...
	it('translates $or operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE json_extract(value, '$.status') = ? OR json_extract(value, '$.status') = ?)");
	...
	it('translates $nor operator', () => {
		...
		expect(result).not.toBeNull();
		expect(result?.sql).toBe("EXISTS (SELECT 1 FROM jsonb_each(data) WHERE NOT (json_extract(value, '$.status') = ? OR json_extract(value, '$.status') = ?))");
